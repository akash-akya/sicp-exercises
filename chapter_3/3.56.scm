...
;; whether it has any factors other than 2, 3, and 5. But this is very
;; inefficient, since, as the integers get larger, fewer and fewer of
;; them fit the requirement. As an alternative, let us call the
;; required stream of numbers S and notice the following facts about
;; it.

;; 1. S begins with 1.
;; 2. The elements of (scale-stream S 2) are also elements of S.
;; 3. The same is true for (scale-stream S 3) and (scale-stream S 5).
;; 4. These are all the elements of S.

;; Now all we have to do is combine elements from these sources. For
;; this we define a procedure merge that combines two ordered streams
...
;; test
(define (test)
  (test-equal (stream-ref S 4) 5)
  (test-equal (stream-ref S 1) 2)
  (test-equal (stream-ref S 10) 15)
  (test-equal (stream-ref S 15) 25))
