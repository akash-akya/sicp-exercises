...
;; computing the area of a region of space described by a predicate P
;; ( x , y ) that is true for points ( x , y ) in the region and false
;; for points not in the region. For example, the region contained
;; within a circle of radius 3 centered at (5, 7) is described by the
...
;; determine whether the point lies in the region. If we try this with
;; many points, then the fraction of points that fall in the region
...
;; monte-carlo procedure that was used above to estimate π . Use your
;; estimate-integral to produce an estimate of π by measuring the area of
;; a unit circle.
...
;; 1.2.6, which returns a nonnegative number less than its input.136
...
(define (create-experiment P x1 x2 y1 y2)
  (lambda ()
...
(define (estimate-integral P x1 x2 y1 y2 trials)
  (* (area x1 x2 y1 y2)
     (monte-carlo trials (create-experiment P x1 x2 y1 y2))))
...
(define (estimate-pi)
                                       ;; cx   cy   radius  x1  x2   y1  y2   trials
  (/ (estimate-integral (create-predicate 1000 1000 1000)   0   2000 0   2000 1000000)
     (square 1000.0)))
