...
;; Exercise 5.18: Modify the make-register procedure of 5.2.1 so that
...
;; contents of the register, and the new contents being
;; assigned. Extend the interface to the machine model to permit you
              ...
              (let ((inst (car insts)))
                (if (label-instruction? inst)
                    ...
                    (begin
                      ...
                      ((instruction-execution-proc inst))
                      (set! instruction-exec-count
                        (+ 1 instruction-exec-count))))
                (execute)))))
...
(define gcd-machine
  (make-machine
   ...
   '((perform (op register-trace-on) (const b))
     test-b
     ...
     (assign a (reg b))
     (assign b (reg t))
     (perform (op register-trace-off) (const b))
     (goto (label test-b))
     gcd-done)))
