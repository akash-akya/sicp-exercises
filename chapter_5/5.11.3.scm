...
;; Stack
(define (make-stack regs)
  (let ((s (map (lambda (r) (list r '())) regs)))
    (define (set-reg-stack stack reg value)
      (cond ((null? stack) (error "Reg not initialized in the stack" reg))
            ((eq? reg (caar stack)) (set-car! stack (cons reg (cons value (cdar stack)))))
            (else (set-reg-stack (cdr stack) reg value))))

    (define (push reg x)
      (set-reg-stack s reg x))

    (define (pop-reg stack reg)
      (cond ((null? stack) (error "Empty stack: POP" s))
            ((eq? reg (caar stack))
             (if (null? (cdar stack))
                 (error "Empty stack: POP" stack)
                 (let ((top (car (cdar stack))))
                   (set-car! stack (cons reg (cdr s)))
                   top)))
            (else (pop-reg (cdr stack) reg))))

    (define (pop reg)
      (pop-reg s reg))

    (define (initialize)
      (set! s (map (lambda (r) (list r '())) regs))
      'done)
    (define (dispatch message)
...
(define (validate-operands operands)
  (map (lambda (operand)
         (if (label-exp? operand)
             (error "Operand can not be label" operand)))
       operands))
...
(define (test)
  ...
  (set-register-contents! gcd-machine 'b 20)
  (start gcd-machine)
  (test-equal (get-register-contents gcd-machine 'a) 10)
  (test-equal (get-register-contents gcd-machine 'b) 20))
