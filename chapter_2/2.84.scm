...
;; the apply-generic procedure so that it coerces its arguments to have
;; the same type by the method of successive raising, as discussed in
;; this section. You will need to devise a way to test which of two types
;; is higher in the tower. Do this in a manner that is “compatible” with
...
(test-equal (level (make-scheme-number 10)) 2)
(test-equal (level (make-rational 20 10)) 1)
(test-equal (level (make-complex-from-real-imag 10 40)) 0)

(test-equal (add (make-rational 10 2)
                 (make-scheme-number 20))
  (make-rational 25 1))

(test-equal (add (make-scheme-number 20)
                 (make-rational 10 2))
  (make-rational 25 1))

(test-equal (add (make-scheme-number 20)
                 (make-complex-from-mag-ang 10 0))
  (make-complex-from-real-imag 30 0))

(test-equal (add (make-rational 20 3)
                 (make-complex-from-mag-ang 10 0))
  (make-complex-from-real-imag (+ (/ 20 3) 10) 0))

(test-equal (add (make-complex-from-mag-ang 10 0)
                 (make-rational 20 3))
  (make-complex-from-real-imag (+ (/ 20 3) 10) 0))
