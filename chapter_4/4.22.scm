...
;; special form let. (See Exercise 4.6.)
...
(define (analyze-assignment exp)
  (let ((var (assignment-variable exp))
    ...
    (lambda (env)
      (set-variable-value! var (vproc env) env) 'ok)))
...
(define (analyze-sequence exps)
  ...
  (define (loop first-proc rest-procs)
    (if (null? rest-procs) first-proc (loop (sequentially first-proc (car rest-procs))
                                            (cdr rest-procs))))
...
(define (eval-assignment exp env)
  (set-variable-value! (assignment-variable exp)
                       (my-eval (assignment-value exp) env) env) 'ok)

(define (eval-definition exp env)
  (define-variable! (definition-variable exp)
    (my-eval (definition-value exp) env) env) 'ok)
...
(define (setup-environment)
  (let ((initial-env (extend-environment (primitive-procedure-names)
                                         (primitive-procedure-objects) the-empty-environment)))
...
(define (analyze-let exp)
  ...
  (define (let-body exp)
    (cddr exp))
  (let ((vars (map car (bindings exp)))
        (args (map cadr (bindings exp)))
        (body (let-body exp)))
    (analyze-application (cons (make-lambda vars body) args))))
...
(define (test)
  (test-equal (test-eval '(let ((a (+ 10 10))
                                (b (+ 20 10)))
                            (+ a b))) 50))
