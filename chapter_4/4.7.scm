...
;; Exercise 4.7: Let* is similar to let, except that the bindings of
  ...
  (define (transform bindings body)
    (if (null? bindings)
        ...
        (make-let (car bindings)
                  (transform (cdr bindings) body))))
  (let ((bindings (cadr exp))
        (body (cons 'begin (cddr exp))))
    (transform bindings body)))

(define (let*-action exp env)
  (my-eval (let*->nested-lets exp) env))

(install-from 'let* let*-action)
