...
;; constructs, such as do, for, while, and until. In Scheme, iterative
;; processes can be expressed in terms of ordinary procedure calls, so
;; special iteration constructs provide no essential gain in
;; computational power. On the other hand, such constructs are often
;; convenient. Design some iteration constructs, give examples of
...
;; solution
(define (while-form exp env)
  (let ((predicate (cadr exp))
        (body (cons 'begin (cddr exp))))
    (cond ((true? (my-eval predicate env))
           (my-eval body env)
           (while-form exp env))
          (else nil))))

(install-from 'while while-form)
