...
;; ⟨var⟩ is bound within ⟨body⟩ to a procedure whose body is ⟨body⟩
;; and whose parameters are the variables in the ⟨bindings⟩. Thus, one
;; can repeatedly execute the ⟨body⟩ by invoking the procedure named
;; ⟨var⟩. For example, the iterative Fibonacci procedure (1.2.2) can
;; be rewritten using named let as follows:
...
(define (let-form exp env)
  ...
  (define (get-procedure params body env)
    (my-eval (make-lambda params body) env))

  (define (let-helper name exp env)
    (let* ((bindings (car exp))
           (body (cdr exp))
           (proc (get-procedure (map car bindings) body env))
           (args (map cadr bindings)))
      (if name
          (define-variable! name proc env))
      (my-apply proc (list-of-values args env))))

  (if (named-let? exp)
      (let-helper (cadr exp) (cddr exp) env)
      (let-helper nil (cdr exp) env)))
