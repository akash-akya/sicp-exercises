...
(define (arrow-case? expr)
  (eq? '=> (cadr expr)))

;; Will break for predicate with side-effects
(define (make-cond-arrow predicate consequent rest)
  (make-if predicate
           (list consequent predicate)
           rest))
  ...
  (if (null? clauses)
      ...
      (let ((first (car clauses))
        ...
        (cond  ((cond-else-clause? first)
               ...
               ((arrow-case? first)
                (make-cond-arrow (cond-predicate first)
                                 (caddr first)
                                 (expand-clauses rest)))
