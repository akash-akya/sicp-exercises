...
;; is a name-value pair. Rewrite the environment operations to use
...
;; test
(define (test-frame-primitive)
  (test-equal (make-frame '(a b c) '(1 2 3))
    '((a . 1) (b . 2) (c . 3)))
  (let ((frame (make-frame '(a b c) '(1 2 3))))
    (test-equal (frame-variables frame) '(a b c))
    (test-equal (frame-values frame) '(1 2 3))
    (add-binding-to-frame! 'd 4 frame)
    (test-equal frame '((a . 1) (d . 4) (b . 2) (c . 3)))))
